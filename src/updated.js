...
class NeuralNetworkModel {
  constructor(inputSize, hiddenSize, outputSize, activationFunction) {
  ...
  calculateOutput(hiddenLayerOutput) {
    ...
    for (let j = 0; j < this.outputSize; j++) {
      ...
      output.push(this.activateX(summationX));
    }
    return output;
  }
...
function ConstructGame() {
  ...
  function trainModel() {
    ...
    for (let epochNum = 0; epochNum < epoch; epochNum++) {
      ...
      let totalError = neuralNetwork.current.calculate_mse(dataSet);
      if (totalError <= goal) {
        console.log("Done");
